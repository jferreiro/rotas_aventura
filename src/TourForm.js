...
function TourForm({ addTour }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const requiredFields = ['name', 'location', 'difficultyLevel', 'description'];
    let valid = true;
    ...
    requiredFields.forEach(field => {
      if (!tour[field]) {
        ...
        document.querySelector(`textarea[name="${field}"]`)?.classList.add('error');
        document.querySelector(`select[name="${field}"]`)?.classList.add('error');
      } else {
        document.querySelector(`input[name="${field}"]`)?.classList.remove('error');
        document.querySelector(`textarea[name="${field}"]`)?.classList.remove('error');
        document.querySelector(`select[name="${field}"]`)?.classList.remove('error');
      }
    });
